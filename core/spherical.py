...
import torch
import math
from core.projection_prim_ortho import *
...
def calculate_great_circle_distance(pre_flow, gt_flow, method='Haversine', R=1):
    ...
    startpoint = generate_plane_grid(pre_flow.shape, device=pre_flow.device, dtype=pre_flow.dtype)
...
def calculate_veclen_spherical(flow, R=1):
    ...
    startpoint = generate_plane_grid(flow.shape, device=flow.device, dtype=flow.dtype)
