...
from core.my_cycle_sample import cycle_grid_sample


def generate_plane_grid(tensor_size, device=None, dtype=None):
    B, _, H, W = tensor_size
    if device is None:
    ...
    plane_grid = torch.cat((xx, yy), 1).to(dtype=dtype)
    return plane_grid


def generate_rotation_metrix(axis_list=None, theta_list=None, device=None, dtype=None):
    ...
    for axis, theta in zip(axis_list, theta_list):
        cos_theta = torch.cos(torch.tensor(theta, device=device, dtype=dtype))
        sin_theta = torch.sin(torch.tensor(theta, device=device, dtype=dtype))
        if axis == 'x':
            R_axis = torch.eye(3, device=device, dtype=dtype)
            R_axis[1, 1] = cos_theta
            R_axis[1, 2] = -sin_theta
            R_axis[2, 1] = sin_theta
            R_axis[2, 2] = cos_theta
            R = R @ R_axis
        elif axis == 'y':
            R_axis = torch.eye(3, device=device, dtype=dtype)
            R_axis[0, 0] = cos_theta
            R_axis[0, 2] = sin_theta
            R_axis[2, 0] = -sin_theta
            R_axis[2, 2] = cos_theta
            R = R @ R_axis
        elif axis == 'z':
            R_axis = torch.eye(3, device=device, dtype=dtype)
            R_axis[0, 0] = cos_theta
            R_axis[0, 1] = -sin_theta
            R_axis[1, 0] = sin_theta
            R_axis[1, 1] = cos_theta
            R = R @ R_axis
    return R
...
def Cartesian2Spherical(Cartesian_grid):
    ...
    return Spherical_grid.to(Cartesian_grid)
...
def cycle_interpolate(input_tensor, grid):
    ...
    mask = torch.ones_like(input_tensor_padding)
    mask = F.grid_sample(mask, grid, align_corners=True)
...
class ERP:
    ...
    def plane2spherical(cls, plane_grid):
        ...
        return Spherical_grid.to(plane_grid)
...
def generate_samplegrid(tensor_size, rotate_metrix):
    B, _, H, W = tensor_size
    plane_grid = generate_plane_grid(tensor_size, device=rotate_metrix.device, dtype=rotate_metrix.dtype)
...
def flow2camera(flow_worldgrid, rotate_metrix):
    start_worldgrid = generate_plane_grid(flow_worldgrid.shape, device=flow_worldgrid.device, dtype=flow_worldgrid.dtype)
...
def generate_samplegrid_theta(tensor_size1, tensor_size2, delta_theta):
    ...
    plane_grid = generate_plane_grid(tensor_size1, device=rotate_matrix.device, dtype=rotate_matrix.dtype)
    sphrical_grid = ERP.plane2spherical(plane_grid)
    cartesian_grid = Spherical2Cartesian(sphrical_grid)
...
def img_rotate(image, EulerAngles_zyx=None, sample_grid=None):
    if sample_grid is None:
        ...
        rotate_matrix = generate_rotation_metrix(theta_list=EulerAngles_zyx, device=image.device, dtype=image.dtype)
        sample_grid = generate_samplegrid(image.shape, rotate_matrix)
    # image_rotate = cycle_grid_sample(image, sample_grid, is_grid=False)
    sample_grid = sample_grid.to(device=image.device, dtype=image.dtype)
...
def flo_rotate(flow, EulerAngles_zyx=None, sample_grid_W2C=None, sample_grid_C2W=None):
    ...
    if sample_grid_W2C is None:
        assert EulerAngles_zyx is not None
        rotate_matrix = generate_rotation_metrix(theta_list=EulerAngles_zyx, device=flow.device, dtype=flow.dtype)
        sample_grid_W2C = generate_samplegrid(flow.shape, rotate_matrix.T)
    start_grid_W = generate_plane_grid(flow.shape, device=flow.device, dtype=flow.dtype)
...
def coord_rotate(coords, EulerAngles_zyx):
    rotate_matrix = generate_rotation_metrix(theta_list=EulerAngles_zyx, device=coords.device, dtype=coords.dtype)
