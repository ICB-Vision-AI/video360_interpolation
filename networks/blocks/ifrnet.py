...
def deform_convrelu(in_channels, out_channels, kernel_size=3, stride=1, padding=1, dilation=1, groups=1, bias=True):
    # Fallback to a standard convolution with identical parameters
    return convrelu(
        in_channels,
        out_channels,
...
class Encoder(nn.Module):
    ...
    def forward(self, in_x):
        is_list = isinstance(in_x, (list, tuple))
        if is_list:
            split_sizes = [inp.shape[0] for inp in in_x]
            in_x = torch.cat(in_x, dim=0)
        ...
        if is_list:
            per_input = [[] for _ in split_sizes]
            for feat in fs:
                chunks = torch.split(feat, split_sizes, dim=0)
                for i, chunk in enumerate(chunks):
                    per_input[i].append(chunk)
            return [tuple(features) for features in per_input]
...
class InitDecoder(nn.Module):
    def __init__(self, in_ch, out_ch, skip_ch) -> None:
        ...
        self.convblock = nn.Sequential(
            ...
            nn.ConvTranspose2d(in_ch * 2, out_ch + 4, kernel_size=4, stride=2, padding=1, bias=True),
        )
...
class IntermediateDecoder(nn.Module):
    def __init__(self, in_ch, out_ch, skip_ch) -> None:
        ...
        self.convblock = nn.Sequential(
            ...
            nn.ConvTranspose2d(in_ch * 3, out_ch + 4, kernel_size=4, stride=2, padding=1, bias=True),
        )
