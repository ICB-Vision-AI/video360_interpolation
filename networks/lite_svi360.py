...
from networks.blocks.feat_enc import BasicEncoder, SmallEncoder
...
from networks.blocks.raft import BasicUpdateBlock, SmallUpdateBlock
...
class Model(nn.Module):
    """
    Lite dual-branch AMT variant: primitive branch (AMT-L style) and orthogonal branch (AMT-S style) interleaved at each decoder stage.
    """
    ...
    def __init__(
        ...
        # Primitive components (AMT-L style)
        prim_channels = [48, 64, 72, 128]
        prim_skip_channels = 48
        self.prim_feat_encoder = BasicEncoder(output_dim=128, norm_fn="instance", dropout=0.0)
        ...
        # Orthogonal branch (AMT-S style)
        ortho_channels = [20, 32, 44, 56]
        ortho_skip_channels = 20
        self.ortho_feat_encoder = SmallEncoder(output_dim=84, norm_fn="instance", dropout=0.0)
    ...
    def _get_ortho_updateblock(self, cdim, scale_factor=None):
        return SmallUpdateBlock(cdim=cdim, hidden_dim=76, flow_dim=20,
                                corr_dim=64, fc_dim=68,
                                scale_factor=scale_factor, corr_levels=self.corr_levels,
                                radius=self.radius)

    def _get_spherical_update_block(self, cdim, scale_factor=None):
        return SphericalUpdateBlock(cdim=cdim, hidden_dim=128, flow_dim=48,
                                    corr_dim=256, corr_dim2=160, fc_dim=124,
